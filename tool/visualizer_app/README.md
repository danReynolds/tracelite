...
artifacts, runs doctor over the downloaded artifacts, and can publish them to a
draft GitHub release from a tag or manual dispatch. Hosted release packaging
skips only the tagged heavyweight dense-trace widget stress test; run
`flutter test` locally for full stress coverage. Set `sign_macos=true` only
after configuring
